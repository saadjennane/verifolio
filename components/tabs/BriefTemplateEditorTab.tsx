...
interface BriefTemplateEditorTabProps {
  templateId: string;
}

export function BriefTemplateEditorTab({ templateId }: BriefTemplateEditorTabProps) {
  ...
  const syncQuestions = async (
    ...
    for (const questionId of deletedIds) {
      await fetch(`/api/briefs/templates/${templateId}/questions/${questionId}`, {
        method: 'DELETE',
      });
    }
    ...
    for (const question of addedQuestions) {
      const res = await fetch(`/api/briefs/templates/${templateId}/questions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: question.type,
    ...
    for (const question of updatedQuestions) {
      ...
      if (
        ...
        await fetch(`/api/briefs/templates/${templateId}/questions/${question.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            type: question.type,
