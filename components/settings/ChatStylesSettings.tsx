      ...
      <div className="flex-1 p-4 space-y-3 overflow-y-auto">
        ...
        <div className="bg-muted rounded-lg border border-border p-3">
          ...
          <div className="space-y-1 text-sm">
            {demoSteps.map((step, i) => (
              <div key={i} className={`flex items-center gap-2 ${step.status === 'completed' ? 'text-muted-foreground' : 'text-foreground'}`}>
        ...
        <div className="flex flex-wrap gap-2">
          {demoSuggestions.map((s, i) => (
            <span key={i} className="px-3 py-1.5 text-sm bg-muted hover:bg-muted/80 rounded-full text-foreground cursor-pointer transition-colors">
              {s}
            </span>
          ))}
        </div>
...
function BubbleChat() {
  return (
    <div className="flex flex-col h-full bg-gradient-to-b from-muted/50 to-muted/30">
      <div className="flex-1 p-4 space-y-2 overflow-y-auto">
        {demoMessages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              <p className="whitespace-pre-wrap">{msg.content}</p>
              <p className={`text-[10px] mt-1 ${msg.role === 'user' ? 'text-blue-100' : 'text-muted-foreground'}`}>
                {msg.time}
              </p>
            </div>
          </div>
        ...
        <div className="flex justify-start">
          <div className="bg-card rounded-[20px] rounded-bl-[4px] px-4 py-3 shadow-sm max-w-[75%]">
        ...
        <div className="flex justify-center flex-wrap gap-2 pt-2">
          {demoSuggestions.map((s, i) => (
            <button
              key={i}
              className="px-4 py-2 text-sm bg-card hover:bg-muted rounded-full shadow-sm text-blue-600 dark:text-blue-400 font-medium transition-all hover:scale-105"
            >
              {s}
            </button>
          ))}
...
function SlackChat() {
  return (
    <div className="flex flex-col h-full bg-background">
      <div className="flex-1 p-4 space-y-4 overflow-y-auto">
        {demoMessages.map((msg, i) => (
          <div key={i} className="flex gap-3 hover:bg-muted/50 -mx-2 px-2 py-1 rounded transition-colors">
            {/* Avatar Slack */}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-foreground/90 whitespace-pre-wrap mt-0.5">{msg.content}</p>
            </div>
          </div>
        ))}
        ...
        <div className="flex gap-3 -mx-2 px-2 py-1">
          ...
          <div className="flex-1">
            <div className="flex items-baseline gap-2">
              <span className="font-bold text-foreground text-sm">Verifolio Bot</span>
              <span className="text-xs text-muted-foreground">10:43</span>
            </div>
            <div className="mt-1 p-3 bg-muted rounded-lg border-l-4 border-blue-500">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">
                Traitement en cours
              </div>
              {demoSteps.map((step, i) => (
                <div key={i} className={`flex items-center gap-2 text-sm py-0.5 ${step.status === 'completed' ? 'text-muted-foreground' : 'text-foreground'}`}>
                  {step.status === 'completed' ? (
                    <span className="text-green-600">✓</span>
                  ) : step.status === 'in_progress' ? (
        ...
        <div className="flex gap-2 ml-12">
          {demoSuggestions.map((s, i) => (
            <button
              key={i}
              className="px-3 py-1.5 text-sm bg-card border border-border hover:border-foreground/30 hover:bg-muted rounded-md text-foreground font-medium transition-colors"
            >
              {s}
            </button>
          ))}
      ...
      <div className="border-t border-border p-3">
        <div className="border border-border rounded-lg focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all">
          <textarea
            placeholder="Envoyer un message à #général"
            className="w-full px-3 py-2 text-sm bg-transparent text-foreground focus:outline-none resize-none"
            rows={1}
            readOnly
          />
...
// COMPOSANT PRINCIPAL
// ============================================
export function ChatStylesSettings() {
