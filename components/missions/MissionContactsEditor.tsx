...
interface MissionContactsEditorProps {
  missionId: string;
  onUpdate?: () => void;
}
...
export function MissionContactsEditor({ missionId, onUpdate }: MissionContactsEditorProps) {
  const [missionContacts, setMissionContacts] = useState<MissionContact[]>([]);
  ...
  async function loadContacts() {
    setLoading(true);
    try {
      const res = await fetch(`/api/missions/${missionId}/contacts`);
      if (res.ok) {
        const data = await res.json();
        const fetchedMissionContacts = data.missionContacts || [];
        const fetchedClientContacts = data.clientContacts || [];

        setMissionContacts(fetchedMissionContacts);
        setClientContacts(fetchedClientContacts);

        // Si aucun contact lié à la mission mais des contacts client existent,
        // auto-lier tous les contacts du client
        if (fetchedMissionContacts.length === 0 && fetchedClientContacts.length > 0) {
        ...
        const linkedIds = new Set<string>(
          fetchedMissionContacts.map((mc: MissionContact) => mc.contact_id)
        );
  ...
  function cancelEdit() {
    const linkedIds = new Set<string>(
      missionContacts.map(mc => mc.contact_id)
    );
    setSelectedIds(linkedIds);
    setEditing(false);
  }
      ...
      <div className="space-y-4">
        ...
        {clientContacts.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 py-4 text-center">
            ...
            <br />
            Ajoutez des contacts au client pour pouvoir les lier à la mission.
          </p>
        )}
      ...
      {missionContacts.length > 0 ? (
        <div className="space-y-2">
          {missionContacts.map((mc) => {
            if (!mc.contact) return null;

            const clientContact = clientContacts.find(cc => cc.contact_id === mc.contact_id);
            ...
            return (
              ...
              >
                <div className="flex-1">
                  ...
                  {mc.contact.email && (
                    <p className="text-sm text-gray-500">{mc.contact.email}</p>
                  )}
                  {responsibilities.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
        ...
        <p className="text-sm text-gray-500">Aucun contact lié à cette mission</p>
      )}
