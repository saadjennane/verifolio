...
function NewSupplierQuoteContent() {
  ...
  async function loadData() {
    try {
      ...
      if (suppliersRes.ok) {
        ...
        if (supplierId) {
          const supplier = (data.data || []).find((s: Supplier) => s.id === supplierId);
          if (supplier) {
      ...
      // Load consultations
      const consultationsRes = await fetch('/api/suppliers/consultations?status=open');
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (data.data) {
        ...
        setForm(prev => ({
          ...prev,
          reference: ocr.reference || prev.reference,
          date_devis: ocr.date_devis || prev.date_devis,
          date_validite: ocr.date_validite || prev.date_validite,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      openTab({
        type: 'supplier-quote',
        path: `/suppliers/quotes/${data.data.id}`,
        title: data.data.reference || 'Devis',
  ...
  return (
    <div className="p-6 max-w-3xl">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Nouveau devis fournisseur</h1>
      ...
      <Card className="mb-6">
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Uploadez une image ou un PDF du devis pour extraction automatique des informations.
          </p>
      ...
      <Card>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            {ocrData?.supplier_name && !form.supplier_id && (
              <p className="mt-1 text-sm text-amber-600">
                Fournisseur détecté : {ocrData.supplier_name}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, date_devis: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
