-- Migration 037: Complete fix for all badge-related functions and triggers
-- This migration completely rewrites all buggy functions that reference deal_documents.invoice_id
...
-- =============================================================================
-- 2. RECREATE deal_is_late FUNCTION (CORRECTED)
-- =============================================================================
...
BEGIN
  ...
  -- Note: tasks utilise entity_type/entity_id, pas deal_id directement
  SELECT EXISTS (
    SELECT 1 FROM tasks
  ...
  -- IMPORTANT: Les factures sont liées aux missions, pas aux deals
  SELECT EXISTS (
...
RETURNS TRIGGER AS $$
BEGIN
  -- Tasks utilise entity_type/entity_id, pas deal_id
